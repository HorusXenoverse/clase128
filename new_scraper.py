...
def scrape_more_data(hyperlink):
    ...
    try:
        ...
        for scrape in soup.find_all("tr", attrs={"class": "fact_row"}):
            td_save = scrape.find_all("td")

            for add_temporal in td_save:
                try:
                    temporal_list.append(add_temporal.find_all("div", attrs={"class": "value"}) [0].contents[0] )
        
                except:
                    temporal_list.append("") 
        new_planets_data.append(temporal_list)
...
for row in new_planets_data:
    ...
    for row_recorrer in row:
        sin_linea_salto = row_recorrer.replace("\n", "")
        replaced.append(sin_linea_salto)
